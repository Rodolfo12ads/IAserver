...
class GoldTradingServer:
    ...
    def fetch_external_calendar(self):
        """Busca calendário de fonte pública e salva em CSV"""
        try:
            ...
            # Inspeciona se é lista ou dict
            events = data if isinstance(data, list) else data.get("events", [])
            logger.info(f"[API] {len(events)} eventos recebidos")
            
            # Filtra apenas eventos USD/US
            csv_rows = []
            count_usd = 0
            ...
            for ev in events:
                ...
                csv_rows.append([date, time, "USD", title, impact])
                count_usd += 1
            
            # Salvar em CSV
            with open(self.csv_path, "w", newline="", encoding="utf-8") as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(["Data", "Hora", "Moeda", "Evento", "Impacto"])
                writer.writerows(csv_rows)
            
            logger.info(f"[OK] {count_usd} eventos USD gravados em: {self.csv_path}")
            return True
    ...
    def load_csv_calendar(self):
        ...
        try:
            if not os.path.exists(self.csv_path):
                ...
                logger.info("[INFO] Tentando buscar calendário externo...")
                self.fetch_external_calendar()
                
                if not os.path.exists(self.csv_path):
                    logger.error(f"[ERROR] Não foi possível criar o CSV")
                    return False
        ...
        def update_loop():
            while True:
                try:
                    ...
                    # Buscar calendário externo e atualizar CSV
                    self.fetch_external_calendar()
...
@error_handler
def force_update():
    threading.Thread(target=gold_server.fetch_external_calendar).start()
    ...
    return jsonify({
        ...
        'calendar': 'updating (External API + CSV)',
        'news': news_status
    }), 200
